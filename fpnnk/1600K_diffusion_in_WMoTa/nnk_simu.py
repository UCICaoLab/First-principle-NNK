...
def main():
    ...
    # initial cnn model
    nnk_ml_model = nnk.ml_predict.ml_model(nnk_params.ml_model_weight)

    # dump vacancy
    if nnk_params.dump_vacancy_id == True:
        nnk.utils.dump_id(nnk_params.vacancy_id, 0, nnk_params.f)
   
    #f_neigh_ids = open("nnk.neigh.id", "w")
    #f_barriers = open("nnk.barrier", "w")
    ...
    for step in range(nnk_params.init_step, nnk_params.num_of_steps + nnk_params.init_step):
        ...
        # print(neuron_map_vects.shape)        
        # neural network prediction 
        # energy_barriers = nnk.ml_predict.predict(nnk_params.model_weight, neuron_map_vects)
        energy_barriers = nnk_ml_model.predict(neuron_map_vects).detach().cpu().numpy().flatten()
        ...
        # save neigh id and energy barrier
        #f_neigh_ids.write("{:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f}\n".format(*neigh_ids))
        #f_barriers.write("{:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}\n".format(*energy_barriers))
